...
#This procedure will build 20 SMOTE based XGB models.

n <- 20

#F1-scores for each training set will be stored in a list. 

F1_scores_train <- list()

names <- seq(1,n,length=n)

for (i in c(1:n)){
  
  train.raw <- read.csv(paste0("C:/Users/Mark/Documents/creditcard/smote_samples/train_tomek_smote_",names[i], ".csv", sep=""),stringsAsFactors = FALSE)
  ...
  train_xgb_yhat <- predict(xgb_mdl,newdata = train.raw.origional[,-31],type = "prob")
  ...
  #write.csv(Predictions, paste0("/users/mlennox05/Documents/creditcard/smote_pred/method",i, ".csv", sep=""), row.names=FALSE)
